import styled from 'styled-components';
import React, { useState, useEffect } from 'react';

import {BsThreeDotsVertical} from 'react-icons/bs';
import Image from 'next/image';

import {getCryptoData} from '../pages/api/fetchCoin';

const Coin = ({coin, token}) => {
    const [cryptoTokens, setCryptoTokens] = useState([]);
    useEffect(() => {
        /*const options = {
            method: 'GET',
            headers: {
                'X-RapidAPI-Host': 'coinranking1.p.rapidapi.com',
                'X-RapidAPI-Key': '79f95ac61dmshb80731a069ceea9p155477jsne3fc547fb186'
            }
        };
        
        fetch('https://coinranking1.p.rapidapi.com/coins?referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&tiers=1&orderBy=marketCap&orderDirection=desc&limit=50&offset=0', options)
            .then(response => response.json())
            .then(response => console.log(response))
            .catch(err => console.error(err));
            setCryptoTokens(response=> response.json);
            getCryptoData()
        .then((data) => setCryptoTokens(data));*/
        const Data = getCryptoData().data;
        setCryptoTokens(Data);

    }, [])
        console.log('CryptoTokens: ' + cryptoTokens);
    
    /*
    const [cryptoTokens, setCryptoTokens] = useState([]);
    useEffect(() => {
      const getCryptoTokens = async () => {
          const coins = await fetch (
            "https://coinranking1.p.rapidapi.com/coins?referenceCurrencyUuid=yhjMzLPhuIDl&timePeriod=24h&tiers=1&orderBy=marketCap&orderDirection=desc&limit=50&offset=0"
        )
        const cryptoTokens = (await coins.json()).response
                
                setCryptoTokens(cryptoTokens)
      }
      return getCryptoTokens()
    }, [])

    console.log('CryptoTokens: ', cryptoTokens)
    */
  return (
    <Wrapper>
        <div>
            <div style={{flex:3}} >
                <NameCol>
                    <CoinIcon>
                        <Image src={coin.logo} alt={coin.name} />
                    </CoinIcon>
                    <div>
                        <Primary>{coin.name}</Primary>
                        <Secondary>{coin.sign}</Secondary>
                    </div>
                </NameCol>
            </div>
            <div style={{flex:2}} >
                <Primary>
                    {'$'}
                    {coin.balanceUsd}
                </Primary>
                <Secondary>
                    {coin.balanceCoin}
                </Secondary>
            </div>
            <div style={{flex:1}} >
                <Primary>
                    {'$'}
                    {coin.priceUsd}
                </Primary>
                <div style={{ color: coin.change < 0 ? '#f0616d' : '#26ad75' }} >
                    {coin.change > 0 && '+' }
                    {coin.change}%
                </div>
            </div>
            <div style={{ flex:1 }}>
                {coin.allocation}%
            </div>
            <div style={{ flex:0 }}>
                <BsThreeDotsVertical />
            </div>
        </div>
    </Wrapper>
  )
}

const Wrapper = styled.div`
   display: flex;
   width: 100%;
   justify-content: space-between;
   align-items: center;

   & > div {
       width: 100%;
       display: flex;
       justify-content: space-between;
       align-items: center;
       padding: 1rem 2rem;
   }
`

const NameCol = styled.div`
   display: flex;
   align-items: center;
`

const CoinIcon = styled.div`
   width: 1.8rem;
   margin-right: 1rem;
`

const Primary = styled.div`
   margin-bottom: 0.1rem;
`

const Secondary = styled.div`
   color: #8a919e;
   font-size: 0.8rem; 
`

export default Coin